import { executePlanWithWallet, type WalletProvider } from '@/lib/magicblock'
...
export function createFlaekClient(options: FlaekClientOptions = {}) {
  ...
  async function createJob(input: {
    ...
    }) as Promise<{ job_id: string; status: string; plan?: any }>
  }
  ...
  function createLogReporter(jobId: string, onLog?: (message: string) => void) {
    ...
    const flush = async () => {
      ...
      try {
        ...
        // Ignore log delivery failures to avoid breaking execution
      } finally {
        flushing = false
