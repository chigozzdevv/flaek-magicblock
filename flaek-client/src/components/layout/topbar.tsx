          ...
          {menuOpen && (
            <div className="absolute right-0 mt-2 w-64 rounded-xl border border-white/10 bg-bg-elev shadow-xl overflow-hidden">
              <div className="px-4 py-3 border-b border-white/10">
                ...
                <div className="text-sm font-medium truncate" title={userName || '—'}>
                  {userName || '—'}
                </div>
              </div>
              <div className="px-4 py-3 border-b border-white/10">
                ...
                <div className="flex items-center justify-between mt-1">
                  <div className="text-sm font-medium truncate max-w-[180px]" title={org}>
                    {org || '—'}
                  </div>
                  <button
                    onClick={() => {
                    ...
                    className="w-6 h-6 rounded bg-white/[0.06] border border-white/10 flex items-center justify-center hover:bg-white/[0.1]"
                    title="Rename"
                  >
                    <Pencil size={14} />
                  </button>
        ...
        <form onSubmit={saveName} className="space-y-4">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="inline-flex items-center justify-center rounded-lg bg-gradient-to-r from-brand-500 to-brand-600 px-5 py-2.5 text-sm font-semibold disabled:opacity-50"
            >
              {saving ? 'Saving…' : 'Save'}
            </button>
            <button
